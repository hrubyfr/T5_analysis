...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "r:i:o:")) != -1) {
    ...
    default:
      cerr << "Usage: " << argv[0]
           << " -r <run_number> [-i <input_path>] [-o <output_file>]" << endl;
      return -1;
    }
  }
  ...
  TString txt_sig_x = Form("#sigma_{x} = %.2f mm", sig_x);
  TString txt_sig_y = Form("#sigma_{y} = %.2f mm", sig_y);
  ...
  TLatex *ltx_sigY = new TLatex(sig_x - offset, sig_y - 5.0, txt_sig_y);
