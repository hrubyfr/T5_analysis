...
event_T5_detection TOF_reconstructor::Return_position(
  ...
  for (size_t i = 0; i < hit_pmt_ids.size(); i++) {
    if (hit_mpmt_ids.at(i) == T5_MPMT_ID &&
      ...
      // Take the first trigger event (should be only one, hopefully), and set
      // it as trigger time, then break the for loop
  ...
  for (int i = 0; i < N_T5_SCINTS; i++) {
    ...
    for (const auto &sipm_time_a : T5_times[i]) {
      ...
      for (const auto &sipm_time_b : T5_times[i + 8]) {
        bool time_b_valid = (sipm_time_b < EXPECTED_DETECTION_TIME_MAX &&
        ...
        if (abs(position_x) <= SCINT_DIMENSIONS[i] / 2) {
          ...
          if (_verbose)
            cout << "Hit was reconstructed out of bounds -- due to detector "
                    "smearing?"
                 << endl;
        } else {
          is_valid_hit = false;
          ...
          if (_verbose)
            cout << "Hit was extremely out of bounds, is not valid" << endl;
        }
        // _verbose checks
        if (_verbose)
