    ...
    for (size_t input_idx = 0; input_idx < input_paths.size(); ++input_idx) {
        ...
        while (tree.Next()) {
            ...
            for (size_t j = 0; j < cut.Get_T5_ids().size(); j++) {
                ...
                // hists.fill(Form("T5_number_of_hits_%i", j),
                // sum_hits_T5_i);
                n_hits_in_T5_in_single_event += sum_hits_T5_i;
            }
            // hists.fill("n_event_hits",
            // n_hits_in_T5_in_single_event);

            // for (const auto& hit : detections.T5_hits){
            // 	if (!hit.is_valid_hit || hit.quality !=
            // HitQuality::Perfect){ 		continue;
            // 	}
            // 	// hists.fill("positions", hit.position_x,
            // hit.position_y);
            // }
            all_T5_hits.push_back(detections);
        }
        // auto hist = hists.get_histogram_2D("positions");
        // TF2* gaus_2D = new TF2("gaus_2D", "bigaus",
        // recon.Get_scint_xmin(3), recon.Get_scint_xmax(3),
        // recon.Get_ymin(), recon.Get_ymax());
        ...
        // double rho    = gaus_2D->GetParameter(5); // Correlation
        // factor
        //
        // // 3. Calculate the TRUE mathematical peak height of the
        // bigaus function double denominator = 2.0 * TMath::Pi() *
        // sig_x * sig_y * std::sqrt(1.0 - rho*rho); double
        // peak_amplitude = volume / denominator;
        //
        ...
        //
        // // Let's draw all 3 levels to make it look incredibly
        // professional: double contours[3]; contours[0] =
        // peak_amplitude * std::exp(-4.5); // 3-sigma (widest, lowest)
        // contours[1] = peak_amplitude * std::exp(-2.0); // 2-sigma
        // contours[2] = peak_amplitude * std::exp(-0.5); // 1-sigma
        // (tightest, highest)
        //
        ...
        //
        // double offset = 10.0;
        // TLatex* ltx_sigX = new TLatex(sigma_x - offset, sigma_y,
        // txt_sigma_x); TLatex* ltx_sigY = new TLatex(sigma_x - offset,
        // sigma_y - 5.0, txt_sigma_y);
        //
        // ltx_sigX->SetTextSize(0.04); ltx_sigX->SetTextColor(kBlack);
        // ltx_sigY->SetTextSize(0.04); ltx_sigY->SetTextColor(kBlack);
        //
        // TLatex* contour_sigma = new TLatex(sigma_x - offset, -sigma_y
        // + offset, "1#sigma"); contour_sigma->SetTextSize(0.04);
        ...
        // 	hists. hist_projectX("positions", h_name.Data(), i+1,
        // i+1); 	hists. get_histogram(h_name.Data())->Fit("gaus", "QR",
        // "", recon.Get_scint_xmin(i), recon. Get_scint_xmax(i));
        // }
        // TString plots_directory = "plots/Run_" +
        // std::to_string(run_number); gSystem->Exec("mkdir -p " +
        // plots_directory); gSystem->cd(plots_directory);
        ...
        // 		cout << "Scintillator ID: " <<
        // hit.scintillator_id.value() <<
        // "\t"
        // 		     << "Average time: " << hit.hit_time.value()
        // << "\t"
        // 		     << "First SiPM time: " <<
        // hit.sipm_time_a.value() << "\t"
        // 		     << "Second SiPM time: " <<
        // hit.sipm_time_b.value() << "\t"
        // 		     << endl;
        // 	}
        // }
        //
        // cout << endl;
        // cout << "Printing out events with multiple valid hits: " <<
        // endl; for (const auto& event : multivalidhits_events){ 	cout
        // << "Event " << event.event_nr << ": " << endl; 	for (const
        // auto& hit : event.T5_hits){ 		cout << "Scintillator ID: " <<
        // hit.scintillator_id.value() <<
        // "\t"
        // 		     << "Average time: " << hit.hit_time.value()
        // << "\t"
        ...
        cout << n_T5_valid_events << " events got a valid reconstruction -- "
             << n_pass_cut - n_T5_valid_events << " were mismatched events?"
             << endl
             << n_invalid_hits
             << " events were invalid -- mismatched events (the only "
                "paired SiPM "
             ...
             << n_events_with_multiple_valid_hits_had_one_in_expected_window
             << " of those had at least one hit in the expected time "
                "window"
             << endl
             << n_events_with_multiple_scint_hits
             << " events had hits in multiple scintillators -- in the "
                "expected "
                "time window"
             << endl
             << endl;
