...
event_T5_detection TOF_reconstructor::Return_position(
	...
	for (size_t i = 0; i < hit_mpmt_ids.size(); i++) {
		...
		T5_times.at(SiPM_index)
		    .push_back(hit_pmt_times.at(i) - trigger_time);
	}
	...
	for (int i = 0; i < N_T5_SCINTS; i++) {
		...
		for (const auto &sipm_time_a : T5_times[i]) {
			...
			for (const auto &sipm_time_b : T5_times[i + 8]) {
				...
				double avg_time =
				    (sipm_time_a + sipm_time_b) / 2;
				hit.hit_time = avg_time;
				is_valid_hit = true;
				double position_x =
				    (time_diff - SCINT_BIAS.at(i)) * _v_eff /
				    2.0;
				double position_y = SCINT_Y_POSITIONS[i];
				double uncertainty =
				    sqrt(pow(v_eff_uncertainty, 2) *
					     pow(time_diff / 2, 2) +
					 pow(sigma_sipm_i[i], 2) *
					     pow(_v_eff / 2, 2));
				double margin_of_error = 3 * uncertainty;

				if (abs(position_x) <=
				    SCINT_DIMENSIONS[i] / 2) {
					is_valid_hit = true;
