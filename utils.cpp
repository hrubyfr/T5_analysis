...
void Histograms::book2D(const std::string &name, const std::string &title) {
	auto hist = new TH2D(name.c_str(), title.c_str(), m_default_bins,
			     m_default_min, m_default_max, m_default_bins,
			     m_default_min, m_default_max);
	hist->SetDirectory(nullptr);
	m_hists_map[name] = hist;
}
...
void Histograms::print_all() {
	for (const auto &[name, hist] : m_hists_map) {
		if (std::find(special_hists.begin(), special_hists.end(),
			      name) != special_hists.end())
			continue;
		TString c_name = "c_" + name;
		auto canvas =
		    new TCanvas(c_name, c_name, m_canvas_default_size_x,
				m_canvas_default_size_y);
		if (hist != nullptr) {
...
void Histograms::hist_projectX(const std::string &source_name,
	...
	if (m_hists_map.count(source_name) == 0) {
		std::cerr << "Error: Source histogram '" << source_name
			  << "' not found!\n";
		return;
	}
	...
	// ROOT automatically creates a new TH1D on the heap and returns the
	// pointer
	TH1D *proj = h2->ProjectionX(new_name.c_str(), first_y_bin, last_y_bin);

	// 4. Detach it from ROOT's memory manager and store it in our
	// dictionary
