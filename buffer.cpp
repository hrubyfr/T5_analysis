...
void setup_histograms(Histograms &hists, TOF_reconstructor &recon) {
  ...
  hists.book1D("valid_hit_times",
               "Average time of valid hits;time - trigger[ns];count", 200, -250,
               250);
  ...
  hists.book2D("positions", "Reconstructed T5 positions;x[mm];y[mm];count", 50,
  ...
  for (int i = 0; i < n_scints; i++) {
    hists.book1D(
        Form("positions_%i", i),
        Form("Reconstructed positions in scintillator %i;x[mm];count", i), 25,
        -recon.GetScintDimensionX(i) / 2, recon.GetScintDimensionX(i) / 2);
  }
}
