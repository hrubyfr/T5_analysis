...
TOF_reconstructor::TOF_reconstructor(double v_eff)
    : _v_eff(v_eff), v_eff_uncertainty(33.7644),
      sigma_sipm_i({0.333076, 0.360282, 0.356124, 0.304019, 0.226525, 0.243068,
                    0.306572, 0.273243}),
      sigma_sipm_i_uncertainties({0.0101467, 0.0490062, 0.0703601, 0.0993561,
                                  0.133235, 0.103015, 0.0575791, 0.0123247}),
...
event_T5_detection TOF_reconstructor::Return_position(
    ...
    // Create 16 vectors where to store all the times detected by T5 SiPMs, some
    // will be empty, some will have multiple hits, ideally paired -- later will
    // check for hits in paired detectors
    vector<vector<double>> T5_times(N_T5_SIPMS);
    vector<vector<double>> raw_times(N_T5_SIPMS);
    ...
    for (size_t i = 0; i < hit_pmt_ids.size(); i++) {
        if (hit_mpmt_ids.at(i) == T5_MPMT_ID &&
            ...
            // Take the first trigger event (should be only one, hopefully), and
            // set it as trigger time, then break the for loop
    ...
    for (size_t i = 0; i < hit_mpmt_ids.size(); i++) {
        ...
        raw_times.at(SiPM_index).push_back(hit_pmt_times.at(i));
        T5_times.at(SiPM_index).push_back(hit_pmt_times.at(i) - trigger_time);
        T5_charges.at(SiPM_index).push_back(hit_pmt_charges.at(i));
    }
    // Loop over all saved times in the corresponding vectors, compare all the
    // times
    for (int i = 0; i < N_T5_SCINTS; i++) {
        if (T5_times[i].empty() || T5_times[i + 8].empty())
            continue;
        for (int iter_a = 0; iter_a < T5_times[i].size(); iter_a++) {
            auto sipm_time_a = T5_times[i].at(iter_a);
            auto sipm_charge_a = T5_charges[i].at(iter_a);
            auto raw_time_a = raw_times[i].at(iter_a);
            bool time_a_valid = (sipm_time_a < EXPECTED_DETECTION_TIME_MAX &&
                                 sipm_time_a > EXPECTED_DETECTION_TIME_MIN);
            for (int iter_b = 0; iter_b < T5_times[i + 8].size(); iter_b++) {
                auto sipm_time_b = T5_times[i + 8].at(iter_b);
                auto raw_time_b = raw_times[i + 8].at(iter_b);
                auto sipm_charge_b = T5_charges[i + 8].at(iter_b);
                bool time_b_valid =
                ...
                hit.raw_time_a = raw_time_a;
                hit.raw_time_b = raw_time_b;
                hit.raw_time = (raw_time_a + raw_time_b) / 2;
                hit.sipm_a_charge = sipm_charge_a;
                hit.sipm_b_charge = sipm_charge_b;
                hit.total_hit_charge = sipm_charge_a + sipm_charge_b;
                hit.trigger_time = trigger_time;

                hit.is_in_time_window = time_a_valid && time_b_valid;
                if (!hit.is_in_time_window && _verbose) {
                    cout << "WARNING: SiPM time is out of expected hit times"
                         << endl;
                }
                ...
                if (abs(position_x) <= SCINT_DIMENSIONS[i] / 2) {
                    ...
                    if (_verbose)
                        cout << "Hit was reconstructed out of bounds -- due to "
                                "detector smearing?"
                             << endl;
                } else {
                    is_valid_hit = false;
                    ...
                    if (_verbose)
                        cout << "Hit was extremely out of bounds, is not valid"
                             << endl;
                }
                // _verbose checks
                if (_verbose)
